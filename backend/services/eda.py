...
def build_eda_report(df: pd.DataFrame) -> dict:
    ...
    churn_by_contract = [
        {
            ...
            "label": f"{int(row['Contract_period'])} mês" if int(row['Contract_period']) == 1 else f"{int(row['Contract_period'])} meses",
    ...
    # Page 5 — Chart 2: churn by lifetime cohort (first 30 days window)
    ...
    # Page 6 — protective binary features
    protective_features = [
    ...
    derived = df.assign(
        ratio_freq_atual_vs_lifetime=df["ratio_freq"],
        flag_early_user=df["flag_early_user"],
    )
    numeric_cols = FEATURES + ["ratio_freq_atual_vs_lifetime", "flag_early_user", "Churn"]
    available_cols = [c for c in numeric_cols if c in derived.columns]
    correlations = (
        derived[available_cols]
    ...
    # Basic comparison churn vs. retained
    ...
    # Scatter sample for freq analysis
