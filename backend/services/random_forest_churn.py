...
def train_random_forest(df: pd.DataFrame) -> dict:
    ...
    if y.nunique() < 2 or y.value_counts().min() < 2:
        raise ValueError("A coluna Churn precisa ter exemplos suficientes das classes 0 e 1.")
    ...
    score_distribution = _build_score_distribution(df["churn_score"])

    # ROC curve points (subsample for response size)
...
def _build_score_distribution(scores: pd.Series) -> list[dict]:
    ...
    return [
        {"bucket": str(label), "count": int(counts.get(label, 0)),
         "pct": round(float(counts.get(label, 0) / total), 4)}
        for label in labels
    ]
...
def score_single_customer(model, feature_defaults: dict, data: dict) -> dict:
    from .data_loader import DEFAULT_FEATURE_VALUES  # noqa: PLC0415
    feature_values = [
